...
| 设计版本 | V1.8 |
| 更新日期 | 2026-07-17 |
| 适用基线 | `main` at `d0ee357`，PR #24 合并后 |
| 已完成 | PR A、PR B、PR B.1、PR C、PR D、PR E、PR F、PR G、PR H、PR I、PR J |
| 当前实施项 | PR K：M0 Reliability Gate |
| 下一实施项 | 用户验收 Run Center；通过后进入 VS01 |
...
| R-03 | PR F 已解决并通过重试边界测试 | `max_attempts` 没有自动重试语义 |
| R-04 | PR F 已解决并通过过期回收测试 | 过期 RUNNING 达到上限后无人处理 |
| R-05 | PR H / PR I 已解决并通过故障注入恢复测试 | 崩溃后状态可能分裂 |
| R-06 | PR G 已解决并通过 Provider 契约测试 | Provider Protocol 没有运行时解耦 |
| R-07 | PR H 已解决并通过 Artifact / Blob 幂等测试 | 不同 Task 的 lineage 可能错误共享 |
| R-08 | Heartbeat、Reaper 和 Reconciler 均已实现并测试 | 后端恢复链路已验证 |
...
当前 M0 可靠性目录已无 strict xfail。PR J 和 PR K 仍需完成页面可见性、Windows 强杀记录与最终 Gate。
